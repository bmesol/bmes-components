import React from "react";
import { ToastVariants, ToastProps } from "./DTO's";
import closeIcon from "./../../../assets/Images/close-icon.svg";
import "./Toast.scss";

const Toast = (props: ToastProps) => {
  const { variant, heading = "", message = "", showToast = true, onClose, classNames = "" } = props;
  return (
    <>
      {showToast && (
        <section>
          <div id="toast" className={`${variant} ${classNames}`}>
            <div className={`toast-content d-flex justify-content-between align-items-center`}>
              <div>
                <p className="toast-heading p-0 m-0">{heading}</p>
                <p className="toast-message p-0 m-0">{message}</p>
              </div>
              <button className="bg-transparent border-0 p-0 m-0" onClick={onClose}>
                <img src={closeIcon} />
              </button>
            </div>
          </div>
        </section>
      )}
    </>
  );
};

export { Toast, ToastVariants };